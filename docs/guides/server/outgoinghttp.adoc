...
== Vert.x HTTP client (Preview)

A Vert.x-based HTTP client is available as a preview feature. It replaces the Apache HTTP Client with Vert.x/Netty for all outgoing connections, enabling future PQC (Post-Quantum Cryptography) support via OpenSSL.

To enable it:

<@kc.start parameters="--features=http-client:v2"/>

When enabled, all outgoing HTTP traffic, including identity provider federation, token exchange, backchannel logout, OCSP/CRL validation and SAML artifact resolution, uses the Vert.x HTTP client. All existing configuration options documented below (`socket-timeout-millis`, `max-pooled-per-route`, `proxy-mappings`, `NO_PROXY`, etc.) work the same way with both clients.

=== Extension migration

The `getHttpClient()` method on `HttpClientProvider`, which returns an Apache `CloseableHttpClient`, is deprecated and will be removed in a future release. When the Vert.x client is enabled, `getHttpClient()` returns a compatibility bridge that translates Apache API calls to Vert.x internally.

Extension providers should migrate to one of:

* `SimpleHttp.create(session)` for general HTTP calls (GET, POST, PUT, DELETE with headers, parameters, and JSON/form bodies).
* The provider-neutral helper methods on `HttpClientProvider`: `getString(uri)`, `getInputStream(uri, headers)`, `postText(uri, text)`, `postBinary(uri, body, headers)`.

These APIs work identically with both the Apache and Vert.x clients.
...
Maximum total time for an outgoing HTTP request to complete, in milliseconds. This is a wall-clock cap that limits how long a single request can take regardless of whether data is flowing. Set to 0 to disable. Default: 30000ms
